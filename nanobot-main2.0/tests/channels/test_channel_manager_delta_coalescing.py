...
class TestWebsocketWithoutRegisteredChannel:
    """WebUI may publish websocket outbound events even when no websocket channel is enabled."""

    @pytest.mark.asyncio
    async def test_websocket_outbound_is_ignored_when_channel_not_registered(self, manager, bus):
        await bus.publish_outbound(OutboundMessage(
            channel="websocket",
            chat_id="web:42:abc12345",
            content="",
            metadata={"_turn_end": True, "webui": True},
        ))
        ...
        assert manager.channels["mock"]._send_delta_mock.await_count == 0
