...
class StreamRenderer:
    ...
    def __init__(
        ...
        # Rich Live full-frame redraw is unstable in some Windows terminals once
        # the streamed answer wraps across multiple lines. Fall back to direct
        # incremental writes there instead of repainting the whole buffer.
        self._use_live = self._console.is_terminal and sys.platform != "win32"
    ...
    def _ensure_plain_newline(self) -> None:
        if not self._plain_line_open:
            return
        out = sys.stdout
        out.write("\n")
        out.flush()
    ...
    async def on_delta(self, delta: str) -> None:
        ...
        if not self._use_live:
            ...
            out.write(delta)
            out.flush()
            self._plain_line_open = not delta.endswith("\n")
            return
    ...
    async def on_end(self, *, resuming: bool = False) -> None:
        if self._live:
            ...
            self._live = None
        self._stop_spinner()
        if self._buf.strip() and self._use_live:
