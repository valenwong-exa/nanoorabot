...
call :log "[1/4] Building frontend"
cd /d "%WEB%"
call npm run build
...
call :log "[2/4] Syncing static assets"
...
call :log "[3/4] Checking port %PORT%"
...
cd /d "%ROOT%"
start "AI System Agent WebUI 2.0" cmd /d /k ""%PYTHON_EXE%" -m webui --host 0.0.0.0 --port %PORT% --workspace "%WORKSPACE%" --config "%CONFIG%" --log-level DEBUG --oracle-config "%ORACLE_CONFIG%""
set "PORT_READY="
...
if not defined PORT_READY (
    ...
    echo   "%START_LOG%"
    echo   and the visible "AI System Agent WebUI 2.0" console window.
    goto :error
)
