...
from webui.utils.channel_compat import normalize_transport_channel
...
def _to_info(job) -> CronJobInfo:
    return CronJobInfo(
        ...
        payload=CronPayloadModel(
            ...
            channel=normalize_transport_channel(job.payload.channel),
            to=job.payload.to,
        ),
...
async def create_job(
    ...
    try:
        job = svc.cron.add_job(
            ...
            channel=normalize_transport_channel(body.payload.channel),
            to=body.payload.to,
...
async def list_cron_sessions(
    ...
    def _session_preview(session_info: dict) -> str:
        preview = session_info.get("last_message")
        if preview is None:
    ...
    if search:
        ...
        cron_sessions = [
            s for s in cron_sessions
            if q in s.get("key", "").lower() or q in _session_preview(s).lower()
        ]
    ...
    return [
        SessionInfo(
            ...
            last_message=_session_preview(s),
        )
