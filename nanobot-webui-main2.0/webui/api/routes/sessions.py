...
def _is_own_session(key: str, user: dict) -> bool:
    ...
    if user.get("role") == "admin":
        return True
    return is_webui_session_key(key) and key.startswith(f"web:{user['id']}")


def _session_preview(session_info: dict) -> str | None:
    """Normalize session preview field names across nanobot versions."""
...
async def delete_session(
    ...
    deleted = svc.session_manager.delete_session(key)
    if not deleted:
