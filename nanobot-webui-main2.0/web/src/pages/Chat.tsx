...
import { DatabaseMetadataTree } from "../components/DatabaseMetadataTree";
...
export default function Chat() {
  ...
  const toggleDatabasePanel = () => {
    setExpandedInventoryPanel((prev) => (prev === "database" ? null : "database"));
  };

  const toggleHostPanel = () => {
    setExpandedInventoryPanel((prev) => (prev === "host" ? null : "host"));
  };
  ...
  return (
    <div className={cn(
      ...
      >
        <div className="flex min-h-0 flex-1 flex-col">
          <section
            ...
            style={{ flex: inventoryPanelExpanded ? "1 1 50%" : "1 1 auto" }}
          >
      ...
      {!isMobile && isDatabaseDrawerOpen && (
        <aside
          className="flex w-52 min-w-0 shrink-0 flex-col overflow-hidden rounded-xl bg-card"
          style={{ width: "13rem", minWidth: 0, maxWidth: "13rem", boxShadow: "var(--shadow-card)" }}
        >
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="flex shrink-0 items-center justify-between px-3 py-2">
              <span className="text-sm font-semibold">{t("chat.databasePanelTitle")}</span>
              <button
                type="button"
                onClick={() => {
                  setIsDatabaseDrawerOpen(false);
                }}
                title={t("chat.hideDatabasePanel")}
                className="flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
              >
                <X className="h-4 w-4" />
              </button>
