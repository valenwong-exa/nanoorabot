...
if exist "%RELEASE_ROOT%\runtime\config.webui.json" (
    set "RUNTIME_ROOT=%RELEASE_ROOT%\runtime"
) else (
    set "RUNTIME_ROOT=%ROOT%\runtime"
)
if not defined CONFIG set "CONFIG=%RUNTIME_ROOT%\config.webui.json"
if not defined ORACLE_CONFIG set "ORACLE_CONFIG=%RUNTIME_ROOT%\oracle_config.json"
if not defined TOOL_POLICY set "TOOL_POLICY=%RUNTIME_ROOT%\tool_policy.json"
set "LOG_DIR=%RUNTIME_ROOT%\logs"
set "START_LOG=%LOG_DIR%\start_webui_dba1_main3.log"
if not defined WORKSPACE set "WORKSPACE=%RELEASE_ROOT%\dba1"
if not defined PORT set "PORT=18781"
if not defined PYTHON_EXE set "PYTHON_EXE=%ROOT%\.venv\Scripts\python.exe"

if not exist "%LOG_DIR%" mkdir "%LOG_DIR%"
call :log "==== start_webui_dba1_main3 ===="
call :log "ROOT=%ROOT%"
...
echo [4/4] Starting WebUI on main3.0 core...
call :log "[4/4] Starting WebUI on main3.0 core"
set "PYTHONPATH=%NANOBOT_ROOT%;%PYTHONPATH%"
cd /d "%ROOT%"
start "AI System Agent WebUI main3" cmd /d /k ""%PYTHON_EXE%" -m webui --host 0.0.0.0 --port %PORT% --workspace "%WORKSPACE%" --config "%CONFIG%" --log-level DEBUG --oracle-config "%ORACLE_CONFIG%" --tool-policy "%TOOL_POLICY%" --oracle-audit --oracle-memory"
set "PORT_READY="
...
if not defined PORT_READY (
    call :log "WebUI is not listening on port %PORT% after 20 seconds"
    echo WebUI did not start successfully. Check logs:
    echo   "%START_LOG%"
    echo   and the visible "AI System Agent WebUI main3" console window.
    goto :error
)
...
echo Failed to build or start the WebUI.
exit /b 1
