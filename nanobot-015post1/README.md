...
- 在 memory_cleaner.py 新增了 memory_cleaner 工具，支持 clear_session 、 clear_long_term 、 clear_all ，让 AI 能通过 tool 主动执行“忘记当前记忆 / 清除历史记忆”。
- 在 manager.py 给 SessionManager 增加了清理能力，补了 clear_session() 、 delete_files() ，并增加了清理标记，确保不仅删除磁盘上的 session 文件，也会清掉内存缓存。
- 在 memory.py 给 MemoryStore 增加了 clear_long_term_memory() ，会清空 memory/MEMORY.md 、 memory/history.jsonl ，同时把相关 cursor 重置为 0 。
- 在 loop.py 注册了 memory_cleaner 工具，并把当前 session_key 传给工具，保证它清的是当前会话，不会误清别的会话。
- 还是在 loop.py 里加了“跳过本轮持久化”和“清理后直接收尾”的逻辑，解决了你最初说的那个问题：手工删掉记忆文件后，内存里的旧内容又被写回磁盘。
- 在 router.py 加了命令归一化能力，像“请调用 /forget-session”“执行 /forget-history”这种写法也会被识别成正式斜杠命令。
- 在 builtin.py 新增了固定命令 /forget-session 、 /forget-history 、 /forget-all ，并更新了 /help 文本，让你既可以自然语言触发，也可以显式命令触发。
- 在测试里补了回归验证，包括 test_memory_cleaner_tool.py 、 test_loop_save_turn.py 、 test_memory_store.py 、 test_builtin_forget.py ，确认“清除后不回写”“命令可直达”“长期记忆确实被清空”。
可直接放到 README 的一段话

- 我新增了一个 memory_cleaner 工具和一组固定命令，用来清除当前会话记忆与长期记忆。其中，当前会话记忆会同时从内存缓存和 sessions/*.jsonl 中移除，长期记忆会清空 memory/MEMORY.md 、 memory/history.jsonl 并重置相关 cursor。与此同时，我修改了主循环的持久化逻辑，避免在清除记忆后又把旧内容从内存重新写回磁盘。现在既支持自然语言触发“忘记当前记忆 / 清除历史记忆”，也支持显式命令 /forget-session 、 /forget-history 、 /forget-all 。