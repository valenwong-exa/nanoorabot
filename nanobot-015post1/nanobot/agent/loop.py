...
class _LoopHook(AgentHook):
    ...
    @staticmethod
    def _looks_like_cumulative_snapshot(prev_clean: str, delta_clean: str) -> bool:
        """Detect providers that resend the full visible answer on each chunk."""
        return bool(
            prev_clean
            and delta_clean
            and len(delta_clean) >= len(prev_clean)
            and delta_clean.startswith(prev_clean)
        )
    ...
    async def on_stream(self, context: AgentHookContext, delta: str) -> None:
        ...
        delta_clean = strip_think(delta)

        if self._looks_like_cumulative_snapshot(prev_clean, delta_clean):
            # Some providers emit the full accumulated answer instead of a true
            # token delta. Replacing the raw buffer avoids replaying the prefix.
            self._stream_buf = delta
            new_clean = delta_clean
        else:
            self._stream_buf += delta
...
class AgentLoop:
    ...
    def _register_default_tools(self) -> None:
        ...
        self.tools.register(OpenHtmlTool(workspace=self.workspace, allowed_dir=allowed_dir))
        for cls in (GlobTool, GrepTool):
            self.tools.register(cls(workspace=self.workspace, allowed_dir=allowed_dir))
        self.tools.register(NotebookEditTool(workspace=self.workspace, allowed_dir=allowed_dir))
        self.tools.register(OracleSqlplusTool())
        if self.exec_config.enable:
            ...
            self.tools.register(WinSshLinuxTool())
        if self.web_config.enable:
            self.tools.register(
                WebSearchTool(config=self.web_config.search, proxy=self.web_config.proxy)
            )
            self.tools.register(WebFetchTool(proxy=self.web_config.proxy))
        self.tools.register(MemoryCleanerTool(sessions=self.sessions, store=self.context.memory))
    ...
    def _set_tool_context(
        ...
        if tool := self.tools.get("memory_cleaner"):
            if hasattr(tool, "set_context"):
    ...
    async def _process_message(
        ...
        if msg.channel == "system":
            ...
            if self._finalize_cleared_session(session):
    ...
    def _save_turn(self, session: Session, messages: list[dict], skip: int) -> None:
        ...
        if session.metadata.get(SESSION_SKIP_PERSIST_ONCE_KEY):
            session.metadata.pop(SESSION_SKIP_PERSIST_ONCE_KEY, None)
            return
    ...
    def _finalize_cleared_session(self, session: Session) -> bool:
        """Drop the current session after a tool explicitly asked to forget it."""
        if not session.metadata.get(SESSION_CLEAR_ON_FINALIZE_KEY):
            return False
        session.metadata.pop(SESSION_SKIP_PERSIST_ONCE_KEY, None)
        session.metadata.pop(SESSION_CLEAR_ON_FINALIZE_KEY, None)
        session.clear_state()
        self.sessions.clear_session(session.key, session=session)
        return True
