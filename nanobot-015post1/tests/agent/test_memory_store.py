...
class TestDreamCursor:
    ...
    def test_clear_long_term_memory_resets_memory_history_and_cursors(self, store):
        ...
        assert store.read_file(store._cursor_file).strip() == "0"
        assert store.read_file(store._dream_cursor_file).strip() == "0"
