...
from nanobot.session.manager import SESSION_CLEAR_ON_FINALIZE_KEY, SESSION_SKIP_PERSIST_ONCE_KEY
...
@pytest.mark.asyncio
async def test_loop_hook_deduplicates_cumulative_stream_snapshots(tmp_path: Path) -> None:
    loop = _make_full_loop(tmp_path)
    seen: list[str] = []
    hook = _LoopHook(loop, on_stream=AsyncMock(side_effect=seen.append))
    ctx = AgentHookContext(iteration=0, messages=[])

    await hook.on_stream(ctx, "The open_html tool")
    await hook.on_stream(ctx, "The open_html tool is a built-in function")
    await hook.on_stream(ctx, "The open_html tool is a built-in function that opens URLs.")
...
@pytest.mark.asyncio
async def test_process_message_does_not_rewrite_session_after_forget_flag(tmp_path: Path) -> None:
    loop = _make_full_loop(tmp_path)
    loop.consolidator.maybe_consolidate_by_tokens = AsyncMock(return_value=False)  # type: ignore[method-assign]

    session = loop.sessions.get_or_create("feishu:c-forget")
    session.add_message("user", "old memory")
    loop.sessions.save(session)

    async def fake_run_agent_loop(initial_messages, *, session=None, **_kwargs):
        assert session is not None
        session.clear_state()
        session.metadata[SESSION_SKIP_PERSIST_ONCE_KEY] = True
        session.metadata[SESSION_CLEAR_ON_FINALIZE_KEY] = True
        loop.sessions.invalidate(session.key)
        loop.sessions.delete_files(session.key)
        return (
            "forgotten",
            None,
            [*initial_messages, {"role": "assistant", "content": "forgotten"}],
            "stop",
            False,
        )
    ...
    result = await loop._process_message(
        InboundMessage(channel="feishu", sender_id="u1", chat_id="c-forget", content="forget now")
    )

    assert result is not None
    assert result.content == "forgotten"
    assert not (tmp_path / "sessions" / "feishu_c-forget.jsonl").exists()
    loop.sessions.invalidate("feishu:c-forget")
    reloaded = loop.sessions.get_or_create("feishu:c-forget")
