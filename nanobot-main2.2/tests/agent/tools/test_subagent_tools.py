...
@pytest.mark.asyncio
async def test_subagent_connects_and_cleans_mcp_servers(tmp_path):
    """Subagents should attach configured MCP tools to their isolated registry per run."""
    from nanobot.agent.subagent import SubagentManager, SubagentStatus
    from nanobot.bus.queue import MessageBus
    ...
    mgr = SubagentManager(
        ...
        bus=bus,
        max_tool_result_chars=_MAX_TOOL_RESULT_CHARS,
        tools_config=ToolsConfig(mcp_servers={"hardware": MCPServerConfig(command="hardware-mcp")}),
    )
    mgr._announce_result = AsyncMock()
    ...
    async def fake_run(spec):
        assert "mcp_hardware_" in " ".join(spec.tools.tool_names)
        return SimpleNamespace(
            stop_reason="done",
            final_content="done",
            error=None,
    ...
    with patch("nanobot.agent.subagent.connect_mcp_servers", new=AsyncMock()) as mock_connect:
        async def _connect(servers, registry):
            registry.register(SimpleNamespace(
                name="mcp_hardware_check",
                to_schema=lambda: {"function": {"name": "mcp_hardware_check"}},
            ))
            return {"hardware": fake_stack}
