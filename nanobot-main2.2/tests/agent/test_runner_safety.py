...
@pytest.mark.asyncio
async def test_runner_throttles_repeated_failed_same_file_edit_loops():
    """Only repeated no-progress edits should get a stop-trying hint."""
    from nanobot.agent.runner import AgentRunSpec, AgentRunner
    ...
    repetitive_calls = [
        ToolCallRequest(
            ...
            name="edit_file",
            arguments={
                "path": "quickhealthcheck.sql",
                "old_text": "LOCKED SESSION",
                "new_text": "LOCKED SESSION UPDATED",
            },
        )
        for i in range(5)
    ]
    responses: list[LLMResponse] = [
        LLMResponse(content=f"edit try {i}", tool_calls=[repetitive_calls[i]])
        for i in range(5)
    ]
    responses.append(LLMResponse(content="ok, I will stop retrying and explain", tool_calls=[]))
    ...
    tools.execute = AsyncMock(
        return_value=(
            "Warning: old_text appears 2 times at line 942, line 1067. "
    ...
    assert result.stop_reason == "completed"
    assert result.error is None
    assert result.final_content == "ok, I will stop retrying and explain"
    escalated = [
        ev for ev in result.tool_events
        if ev["status"] == "error"
        and ev["detail"].startswith("file_edit_ambiguous_escalated:")
    ]
    assert escalated, f"expected ambiguous file edit escalation, got: {result.tool_events}"
    tool_messages = [
    ...
    assert tool_messages
    assert any(
        "repeated ambiguous file edit attempts detected" in str(msg.get("content"))
        for msg in tool_messages
    )


@pytest.mark.asyncio
async def test_runner_allows_followup_edits_after_successful_write():
    """A successful write should reset the failed-edit budget for that file."""
    from nanobot.agent.runner import AgentRunSpec, AgentRunner
    ...
    calls = [
        ToolCallRequest(
            id="a1",
            name="edit_file",
            arguments={"path": "quickhealthcheck.sql", "old_text": "A", "new_text": "B"},
        ),
        ToolCallRequest(
            id="a2",
            name="edit_file",
    ...
    responses: list[LLMResponse] = [
        LLMResponse(content=f"step {i}", tool_calls=[calls[i]])
        for i in range(4)
    ]
    ...
    tools.execute = AsyncMock(side_effect=[
        "Warning: old_text appears 2 times at line 100, line 200.",
        "Warning: old_text appears 2 times at line 100, line 200.",
    ...
    assert not any(
        ev["status"] == "error" and ev["detail"].startswith("file_edit_ambiguous_escalated:")
