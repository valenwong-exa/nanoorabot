...
@tool_parameters(
    tool_parameters_schema(
        goal=StringSchema(
            ...
            "Do not use this when the user explicitly asks you to create/spawn a subagent, background "
            "worker, delegated agent, or parallel helper for isolated execution; use `spawn` instead.",
            max_length=12_000,
        ),
...
class LongTaskTool(Tool, _GoalToolsMixin):
    ...
    def description(self) -> str:
        return (
            ...
            "This is for tracking one ongoing objective in the current thread, not for creating a delegated "
            "subagent/background worker; if the user explicitly asks for a subagent or parallel helper, use "
            "`spawn` instead. "
