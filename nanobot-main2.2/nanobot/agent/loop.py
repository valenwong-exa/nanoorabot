...
from nanobot.agent.tool_policy import DangerousToolPolicy, normalize_policy_text
...
_DIRECT_PRINT_HINT = (
    "[System note: The user explicitly requested direct file output. "
    "If you locate or generate a file that satisfies the request, prefer "
    "the just_print_file tool instead of summarizing the file contents.]"
)


def _augment_direct_print_request(text: str) -> str:
    """Append a one-turn hint when the user explicitly mentions /print_file."""
    if "/print_file" not in text.lower():
        return text
    if _DIRECT_PRINT_HINT in text:
        return text
    return f"{text}\n\n{_DIRECT_PRINT_HINT}"
...
class AgentLoop:
    ...
    _PENDING_TOOL_APPROVAL_KEY = "pending_tool_approval"
    _APPROVED_TOOL_CALL_KEY = "approved_tool_call"
    ...
    def __init__(
        ...
        oracle_memory_enabled: bool = False,
        oracle_audit_enabled: bool = False,
        oracle_config_path: str | Path | None = None,
        ...
        self.oracle_memory = oracle_memory or OracleMemoryService(
            audit_enabled=oracle_audit_enabled,
            memory_enabled=oracle_memory_enabled,
            config_path=oracle_config_path,
        )
        if (oracle_memory_enabled or oracle_audit_enabled) and oracle_config_path is None:
            logger.warning("Oracle audit/memory enabled but oracle config path is not configured")
    ...
    def _on_session_message_persisted(self, session: Session, message: dict[str, Any]) -> None:
        self.oracle_memory.publish_session_message(session.key, message)
    ...
    def _build_initial_messages(
        ...
        current_message = msg.content if isinstance(msg.content, str) else ""
        current_message = _augment_direct_print_request(current_message)
        return self.context.build_messages(
            history=history,
            current_message=image_generation_prompt(current_message, msg.metadata),
        ...
        *on_stream_end(resuming)*: called when a streaming session finishes.
        ...
        try:
            result = await self.runner.run(AgentRunSpec(
                ...
                tool_approval_callback=lambda payload: self._set_pending_tool_approval(session, payload) if session is not None else None,
                approved_tool_call=self._approved_tool_call(session),
    ...
    async def close_mcp(self) -> None:
        ...
        await self.oracle_memory.close()
    ...
    async def _process_system_message(
        ...
        messages = self.context.build_messages(
            history=history,
            current_message="" if is_subagent else current_message,
            channel=channel,
            chat_id=chat_id,
    ...
    async def _state_command(self, ctx: TurnContext) -> str:
        ...
        pending_tool_approval = self._pending_tool_approval(ctx.session)
        if pending_tool_approval is not None:
            approval_decision = self._interpret_tool_approval_reply(raw)
            if approval_decision == "approve":
                self._set_approved_tool_call(ctx.session, pending_tool_approval)
                self._clear_pending_tool_approval(ctx.session)
                self.sessions.save(ctx.session)
                approved_note = (
                    f"{raw}\n\n"
                    ...
                    f"Approved tool: {pending_tool_approval.get('tool_name')}\n"
                    "Approved arguments:\n"
                    f"{json.dumps(pending_tool_approval.get('arguments', {}), ensure_ascii=False, indent=2)}"
                )
                ctx.msg = dataclasses.replace(ctx.msg, content=approved_note)
                raw = ctx.msg.content.strip()
            elif approval_decision == "deny":
                self._clear_pending_tool_approval(ctx.session)
                self._clear_approved_tool_call(ctx.session)
                self.sessions.save(ctx.session)
    ...
    def _pending_tool_approval(self, session: Session | None) -> dict[str, Any] | None:
        if session is None:
            return None
        payload = session.metadata.get(self._PENDING_TOOL_APPROVAL_KEY)
        return dict(payload) if isinstance(payload, dict) else None

    def _set_pending_tool_approval(self, session: Session, payload: dict[str, Any]) -> None:
        session.metadata[self._PENDING_TOOL_APPROVAL_KEY] = dict(payload)

    def _clear_pending_tool_approval(self, session: Session) -> None:
        session.metadata.pop(self._PENDING_TOOL_APPROVAL_KEY, None)

    def _approved_tool_call(self, session: Session | None) -> dict[str, Any] | None:
        if session is None:
            return None
        payload = session.metadata.get(self._APPROVED_TOOL_CALL_KEY)
        return dict(payload) if isinstance(payload, dict) else None

    def _set_approved_tool_call(self, session: Session, payload: dict[str, Any]) -> None:
        session.metadata[self._APPROVED_TOOL_CALL_KEY] = {
            "tool_name": payload.get("tool_name"),
            "arguments": dict(payload.get("arguments") or {}),
        }

    def _clear_approved_tool_call(self, session: Session) -> None:
        session.metadata.pop(self._APPROVED_TOOL_CALL_KEY, None)
    ...
    @staticmethod
    def _interpret_tool_approval_reply(raw: str) -> str | None:
    ...
    def _restore_runtime_checkpoint(self, session: Session) -> bool:
        ...
        for restored_message in restored_messages[overlap:]:
            session.append_message(restored_message)
