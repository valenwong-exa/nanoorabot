...
from nanobot.agent.tool_policy import DangerousToolPolicy, ToolPolicyDecision
from nanobot.agent.tools.just_print_file import DirectToolResponse
from nanobot.agent.tools.registry import ToolRegistry
from nanobot.oracle_memory import OracleMemoryService
...
class ToolPolicyInterruption(RuntimeError):
    """Base class for tool policy stops that should surface to the user."""


class ToolPolicyBlockedError(ToolPolicyInterruption):
    """Raised when a tool call is blocked by policy."""


class ToolApprovalRequiredError(ToolPolicyInterruption):
    """Raised when a risky tool call needs explicit user approval."""
...
class AgentRunner:
    ...
    def __init__(
        ...
        audit_sink: OracleMemoryService | None = None,
        tool_policy: DangerousToolPolicy | None = None,
    ):
    ...
    @staticmethod
    def _find_direct_tool_response(
        tool_calls: list[ToolCallRequest],
        results: list[Any],
    ) -> tuple[ToolCallRequest, DirectToolResponse] | None:
        for tool_call, result in zip(tool_calls, results):
            if isinstance(result, DirectToolResponse):
    ...
    async def _run_core(
        ...
        identical_file_read_counts: dict[str, int] = {}
        failed_file_edit_counts: dict[str, int] = {}
        ...
        for iteration in range(spec.max_iterations):
            ...
            if response.should_execute_tools:
                ...
                results, new_events, fatal_error = await self._execute_tools(
                    ...
                    identical_file_read_counts,
                    failed_file_edit_counts,
                )
                ...
                direct_response = self._find_direct_tool_response(response.tool_calls, results)
                if direct_response is not None:
                    direct_call, direct_payload = direct_response
                    if len(response.tool_calls) > 1:
                        error = (
                            "Error: just_print_file must be called alone after locating or "
                    ...
                    tool_message = {
                        "role": "tool",
                        "tool_call_id": direct_call.id,
                        "name": direct_call.name,
                        "content": direct_payload.history_content
                        or f"[Direct file response from {direct_payload.source_path}]",
                    }
                    messages.append(tool_message)

                    clean_direct = hook.finalize_content(context, direct_payload.content) or ""
                    messages.append(build_assistant_message(clean_direct))
                    await self._emit_checkpoint(
                        spec,
                        {
                            "phase": "direct_tool_response",
                            "iteration": iteration,
                            "model": spec.model,
                    ...
                    if hook.wants_streaming():
                        ...
                        await hook.on_stream(context, clean_direct)
                        await hook.on_stream_end(context, resuming=False)
                    final_content = clean_direct
                    stop_reason = "direct_tool_response"
                    context.final_content = final_content
                    context.stop_reason = stop_reason
                    context.error = None
                    await hook.after_iteration(context)
    ...
    async def _request_finalization_retry(
        ...
        *,
        ...
        return await self._request_no_tools(
            ...
            iteration_no=-1,
            llm_call_seq=llm_call_seq,
            request_phase="finalization_retry",
        )
    ...
    async def _try_finalize_after_max_iterations(
        ...
        try:
            response = await self._request_no_tools(
                spec,
                retry_messages,
                iteration_no=spec.max_iterations,
                llm_call_seq=llm_call_seq + 1,
                request_phase="budget_exhausted_finalization",
            )
        except Exception:
            logger.exception(
    ...
    def _publish_audit(
        ...
        *,
        ...
        if sink is None or not hasattr(sink, "publish_audit"):
            return
        usage = self._usage_dict(response.usage if response is not None else None)
        ...
        payload = {
            "session_key": spec.session_key or f"{spec.channel or 'internal'}:{spec.chat_id or 'default'}",
            "turn_id": spec.turn_id or f"{spec.session_key or spec.channel or 'sessionless'}:{llm_call_seq}:{int(request_started_at * 1000)}",
            "iteration_no": iteration_no,
            "llm_call_seq": llm_call_seq,
            "attempt_no": 1,
            "request_phase": request_phase,
            ...
            "finish_reason": response.finish_reason if response is not None else "error",
            "stop_reason": None,
            "error_kind": response.error_kind if response is not None else None,
            "error_type": response.error_type if response is not None else None,
            ...
            "request_tools_json": request_tools,
            ...
            "error_message": None if response is None else response.content if response.finish_reason == "error" else None,
        }
        try:
    ...
    async def _execute_tools(
        ...
        for batch in batches:
            preflight = self._preflight_tool_policy(spec, batch)
            if preflight is not None:
                tool_results.append(preflight)
                break
    ...
    def _preflight_tool_policy(
        self,
        spec: AgentRunSpec,
        batch: list[ToolCallRequest],
    ) -> tuple[str, dict[str, str], BaseException] | None:
        if self.tool_policy is None or not batch:
            return None
        for tool_call in batch:
            if not isinstance(tool_call.arguments, dict):
                continue
            decision = self.tool_policy.evaluate(tool_call.name, tool_call.arguments)
            if decision is None:
                continue
            if (
                decision.action == "warn"
                and DangerousToolPolicy.matches_approved_call(
                    spec.approved_tool_call,
                    tool_call.name,
                    tool_call.arguments,
                )
            ):
            ...
            if decision.action == "block":
                message = self._build_block_message(decision)
                return message, {
                    "name": tool_call.name,
                    "status": "error",
                    "detail": "blocked by dangerous tool policy",
                }, ToolPolicyBlockedError(message)
            if decision.action == "warn":
                if callable(spec.tool_approval_callback):
                    try:
                        spec.tool_approval_callback({
                            "tool_name": tool_call.name,
                            "arguments": tool_call.arguments,
                            "field_path": decision.field_path,
                            "matched_value": decision.matched_value,
                            "rule": {
                                "command": decision.rule.command,
                        ...
                        logger.exception("tool_approval_callback failed")
                message = self._build_warn_message(decision)
                return message, {
                    "name": tool_call.name,
                    "status": "error",
                    "detail": "waiting for user approval",
                }, ToolApprovalRequiredError(message)
        return None
    ...
    async def _run_tool(
        ...
        if isinstance(params, dict):
            policy_interruption = self._check_tool_policy(spec, tool_call, params)
            if policy_interruption is not None:
                return policy_interruption
            file_loop_error = repeated_file_tool_loop_error(
                tool_call.name,
                params,
                identical_file_read_counts,
            )
            if file_loop_error is not None:
                message, detail = file_loop_error
                event = {
                    "name": tool_call.name,
                    "status": "error",
                    "detail": detail,
                ...
                if spec.fail_on_tool_error:
                    return message, event, RuntimeError(message)
        ...
        try:
            ...
            if handled is not None:
                return handled
            failed_edit_error = repeated_failed_file_edit_error(
                tool_call.name,
                params if isinstance(params, dict) else tool_call.arguments,
                payload,
                failed_file_edit_counts,
            )
            if failed_edit_error is not None:
                message, detail = failed_edit_error
                event["detail"] = detail
                if spec.fail_on_tool_error:
                    return message, event, exc
                return message, event, None
            if spec.fail_on_tool_error:
        ...
        if isinstance(result, DirectToolResponse):
            detail = f"direct print {result.source_path}"
        else:
    ...
    def _check_tool_policy(
        ...
        if self.tool_policy is None:
            return None
        decision = self.tool_policy.evaluate(tool_call.name, params)
        if decision is None:
            return None
        if (
            decision.action == "warn"
            and DangerousToolPolicy.matches_approved_call(spec.approved_tool_call, tool_call.name, params)
        ):
            logger.warning(
                "Executing tool after explicit user approval: {} ({})",
                tool_call.name,
                spec.session_key or "default",
            )
            spec.approved_tool_call = None
            return None
        if decision.action == "block":
            message = self._build_block_message(decision)
            return message, {
    ...
    def _build_block_message(decision: ToolPolicyDecision) -> str:
        return (
            ...
            "To proceed, you must update the tool policy first and accept the risk yourself."
        )

    @staticmethod
    def _build_warn_message(decision: ToolPolicyDecision) -> str:
        return (
            "A risky tool call was detected and execution has been paused.\n"
            f"Tool: {decision.tool_name}\n"
            f"Matched rule: {decision.rule.command} ({decision.rule.scope}/{decision.rule.category})\n"
            f"Matched field: {decision.field_path}\n"
            f"Action: warn\n"
            f"Reason: {decision.rule.note or 'Manual confirmation is required'}\n"
            "To continue, reply with `approve` or `同意执行`. To reject it, reply with `cancel` or `拒绝`."
        )
