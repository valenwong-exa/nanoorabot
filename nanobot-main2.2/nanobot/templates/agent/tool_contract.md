...
- For a simple single-file insert or replacement task with a known target, aim for one focused `read_file`, one planned edit, and one verification pass.
...
- Do not bounce between `read_file` and `apply_patch` on the same unchanged snippet. If two edit attempts fail, stop and re-read a wider section before making one consolidated patch.
- Use `grep` to find a stable anchor before editing; do not repeatedly scan and patch the same file without a clear insertion or replacement point.
- Do not run runtime validation commands to compensate for uncertain file edits. First confirm the file content is correct, then run the smallest relevant verification once.

## Single-File Edit Playbook

- When the user asks for a simple change in one known file, first decide the edit shape: insert, replace, append, or delete.
- Find one stable anchor with `grep` or from the initial read. Do not start patching before you know where the change belongs.
- Read only the smallest section that gives enough context to make the edit safely. Read the whole file only when the file is small or structure is unclear.
- Prefer one consolidated patch over many trial patches. If the change is obvious, make it directly instead of probing with multiple near-identical edits.
- After writing, verify by re-reading the changed region or searching for the inserted marker. Run the file or test only after the text on disk is correct.
- If the first edit attempt fails, do not keep tweaking tiny fragments. Re-read a wider region, refresh the anchor, and then make one new patch.

## Edit Anti-Patterns

- Do not read the same unchanged range again and again just to reassure yourself.
- Do not alternate `read_file` -> `apply_patch` -> `read_file` -> `apply_patch` on one file without new information.
- Do not use execution tools to discover whether a patch landed in the right place; inspect the file content first.
- Do not switch to shell file commands as a fallback when the dedicated file tools already provide the needed context.

## Tool Selection Hints

- If the user explicitly wants raw file contents shown verbatim, especially with `/print_file` or wording like "directly display the file", prefer `just_print_file` once the exact file is known instead of summarizing with another LLM turn.
- If the user wants to preview generated HTML or open a local HTML artifact or URL in a browser, prefer `open_html` instead of using `exec` to call `start`, `Start-Process`, or `xdg-open`.
- If the user wants to connect from Windows to a Linux host over OpenSSH with a private key, or upload/download files with SCP, prefer `win_ssh_linux` instead of constructing raw shell commands.
