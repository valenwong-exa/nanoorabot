"""Asynchronous Oracle persistence for audit and session memory records."""
...
class OracleMemoryService:
    ...
    def publish_audit(self, payload: dict[str, Any]) -> None:
        ...
        for key in (
            ...
            normalized[key] = None if normalized.get(key) is None else self._json_dumps(normalized.get(key))
    ...
    def publish_session_message(
        ...
        *,
        ...
        effective_channel, effective_chat_id = self._split_session_key(session_key)
        payload = {
        ...
        if not payload["message_id"] or payload["message_seq"] is None or not payload["role"]:
            return
    ...
    def _load_config_sync(self) -> OracleConfig | None:
        ...
        if not user or not password or not dsn:
            raise RuntimeError("Oracle config is missing user, password or dsn")
    ...
    def _json_dumps(value: Any) -> str:
        return json.dumps(value, ensure_ascii=False, default=OracleMemoryService._json_default)
