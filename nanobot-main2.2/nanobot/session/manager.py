...
class Session:
    ...
    _message_observer: Callable[["Session", dict[str, Any]], None] | None = field(
        default=None,
        repr=False,
        compare=False,
    )
    ...
    def __post_init__(self) -> None:
        ...
        for message in self.messages:
            self._ensure_message_identity(message)
            seq = message.get("message_seq")
            if isinstance(seq, int) and seq > max_seq:
                max_seq = seq
        self.metadata["_message_seq_counter"] = max_seq

    def attach_message_observer(
        self,
        observer: Callable[["Session", dict[str, Any]], None] | None,
    ) -> None:
        self._message_observer = observer

    def _ensure_message_identity(self, message: dict[str, Any]) -> None:
        counter_key = "_message_seq_counter"
        seq = message.get("message_seq")
        if seq is None:
            current = self.metadata.get(counter_key)
            try:
                next_seq = int(current)
            except (TypeError, ValueError):
                next_seq = max(
                    (
                        int(item.get("message_seq"))
                        for item in self.messages
                        if item.get("message_seq") is not None
                    ),
                    default=0,
                )
            next_seq += 1
            message["message_seq"] = next_seq
            self.metadata[counter_key] = next_seq
        else:
            try:
                seq_value = int(seq)
            except (TypeError, ValueError):
            ...
            if seq_value is not None:
                message["message_seq"] = seq_value
                current = self.metadata.get(counter_key)
                try:
                    current_value = int(current)
                except (TypeError, ValueError):
                    current_value = 0
                if seq_value > current_value:
                    self.metadata[counter_key] = seq_value

        if not message.get("message_id"):
            message["message_id"] = uuid.uuid4().hex

    def append_message(self, message: dict[str, Any]) -> dict[str, Any]:
        """Append a fully shaped message to the session."""
        msg = dict(message)
        msg.setdefault("timestamp", datetime.now().isoformat())
        self._ensure_message_identity(msg)
        self.messages.append(msg)
        self.updated_at = datetime.now()
        if self._message_observer is not None:
            try:
                self._message_observer(self, dict(msg))
            except Exception:
                logger.exception("Session message observer failed for {}", self.key)
        return msg
...
class SessionManager:
    ...
    def __init__(self, workspace: Path):
        ...
        self._message_observer: Callable[[Session, dict[str, Any]], None] | None = None
    ...
    def get_or_create(self, key: str) -> Session:
        ...
        if key in self._cache:
            ...
            self._attach_message_observer(session)
            return session
    ...
    def _attach_message_observer(self, session: Session) -> None:
        session.attach_message_observer(self._message_observer)
    ...
    def fork_session_before_user_index(
        ...
        self._attach_message_observer(target)
