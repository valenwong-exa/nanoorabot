...
const ARTIFACT_EXTS = new Set([
  ...
  "md", "markdown",
  "pdf", "zip", "tar", "gz", "csv", "json", "jsonl", "txt", "xml", "yaml", "yml", "toml", "ini", "env",
  "py", "js", "ts", "jsx", "tsx", "css", "scss", "sh", "bash", "sql", "java", "go", "rs", "c", "cpp",
  "mp3", "wav", "flac", "aac",
]);
...
function collectMatches(content: string, pattern: RegExp, seen: Set<string>, results: string[]) {
  ...
  while ((match = pattern.exec(content)) !== null) {
    ...
    if (!ext || !ARTIFACT_EXTS.has(ext) || seen.has(raw)) {
      continue;
    }
    seen.add(raw);
    results.push(raw);
...
export function extractArtifactPaths(content: string): string[] {
  ...
  collectMatches(content, /(?:^|[\s='"(])([~/][^\s'")\]>]+)/gm, seen, results);
  collectMatches(content, /(?:^|[\s='"(])([A-Za-z]:\\[^\s'")\]>]+)/gm, seen, results);
  collectMatches(content, /(?:^|[\s='"(])([A-Za-z]:\/[^\s'")\]>]+)/gm, seen, results);
...
function ToolResultBlock({ message }: { message: ChatMessage }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      {artifactPaths.map((path) => <ArtifactPreview key={path} filePath={path} />)}
    </div>
  );
}
  ...
  if (message.role === "sub_tool") {
    if (artifactOnly) {
      ...
      return (
        <ToolMessageWrapper>
          <div className="space-y-1.5">
            {paths.map((path) => <ArtifactPreview key={path} filePath={path} />)}
          </div>
        </ToolMessageWrapper>
      );
    }
    ...
    <div className={cn("group flex gap-3 px-4", isUser ? "flex-row-reverse" : "flex-row")}>
      ...
      <div className={cn(
        ...
        {isUser
          ...
          : <img src="/icon.png" alt="AI System Agent" className="h-8 w-8 object-cover mix-blend-multiply dark:mix-blend-screen dark:brightness-150" />}
      </div>
